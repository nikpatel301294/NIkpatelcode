...
public class Boss5Script : MonoBehaviour
{
    ...
    public GameObject firobjectsingle;
    public GameObject firobjectdouble;
    ...
    void generate_Bullate()
    {
        ...
        else if (animationmanage == 1)
        {
            ...
            InstantiateBullet1(firepoint1.position.x, firepoint1.position.y);
            InstantiateBullet1(firepoint2.position.x, firepoint2.position.y);
    ...
    void InstantiateBullet(float x, float y)
    {
        ...
        firobjectsingle.GetComponent<bossbullateScipt>().startPosition = startPosition;
        firobjectsingle.GetComponent<bossbullateScipt>().targetPosition = endPosition;

        Instantiate(firobjectsingle, startPosition, Quaternion.identity);


    }
    void InstantiateBullet1(float x, float y)
    {
        startPosition = new Vector3(x, y, 0);
        endPosition = new Vector3(x, y - 10f, 0);

        firobjectdouble.GetComponent<bossbullateScipt>().startPosition = startPosition;
        firobjectdouble.GetComponent<bossbullateScipt>().targetPosition = endPosition;

        Instantiate(firobjectdouble, startPosition, Quaternion.identity);
    ...
    void InstantiateBulletRotate(float x, float y, float z)
    {
        ...
        firobjectsingle.transform.rotation = Quaternion.AngleAxis(rotationAngle, Vector3.down);

        Instantiate(firobjectsingle, startPosition, Quaternion.identity);

        Debug.Log("rotationfind" + firobjectsingle.transform.rotation);
    }
    void resetVaraible()
    {
